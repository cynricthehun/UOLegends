				...
				if ( targeted is Item )
				{
					...
					if ( isGrave == true )
					{
						...
						if ( m_Item.Uses == 0 )
						{
							m_Item.Delete();
							if ( m_From != null )
								m_From.SendMessage( "The duriblity of your shovel has run out." );
						}

						if ( m_From != null )
							m_From.SendMessage( "You begin to dig." );

						DigTimer dt = new DigTimer( m_From, m_Item, TimeSpan.FromSeconds( 10.0 ) );
						dt.Start();
						m_From.PlaySound( Utility.RandomList( 0x125, 0x126 ) );
						m_From.Animate( 11, 1, 1, true, true, 1 );
						m_Item.IsDigging = true;
					}
					else
					{
						if ( m_From != null )
							m_From.SendMessage( "What could possibly be there?" );
					}
