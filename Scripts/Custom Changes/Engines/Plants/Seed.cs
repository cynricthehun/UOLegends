...
namespace Server.Engines.Plants
{
	public class Seed : Item
	{
		...
		private string PlantColorString;
		...
		{
			if ( this.PlantType == PlantType.CampionFlowers )
				PlantTypeString = "campion flowers";
			else if ( this.PlantType == PlantType.Poppies )
				PlantTypeString = "poppies";
			else if ( this.PlantType == PlantType.Snowdrops )
				PlantTypeString = "snowdrops";
			else if ( this.PlantType == PlantType.Bulrushes )
				PlantTypeString = "bulrushes";
			else if ( this.PlantType == PlantType.Lilies )
				PlantTypeString = "lilies";
			else if ( this.PlantType == PlantType.PampasGrass )
				PlantTypeString = "pampas grass";
			else if ( this.PlantType == PlantType.Rushes )
				PlantTypeString = "rushes";
			else if ( this.PlantType == PlantType.ElephantEarPlant )
				PlantTypeString = "elephant ear plant";
			else if ( this.PlantType == PlantType.Fern )
				PlantTypeString = "fern";
			else if ( this.PlantType == PlantType.PonytailPalm )
				PlantTypeString = "ponytail palm";
			else if ( this.PlantType == PlantType.SmallPalm )
				PlantTypeString = "small palm";
			else if ( this.PlantType == PlantType.CenturyPlant )
				PlantTypeString = "century plant";
			else if ( this.PlantType == PlantType.WaterPlant )
				PlantTypeString = "water plant";
			else if ( this.PlantType == PlantType.SnakePlant )
				PlantTypeString = "snake plant";
			else if ( this.PlantType == PlantType.PricklyPearCactus )
				PlantTypeString = "prickly pear cactus";
			else if ( this.PlantType == PlantType.TribarrelCactus )
				PlantTypeString = "tri-barrel cactus";

			if ( m_ShowType )
			{
				if ( this.PlantType != PlantType.ElephantEarPlant )
					LabelTo( from, "a {0} seed", PlantTypeString );
				else
					LabelTo( from, "an {0} seed", PlantTypeString );
			}
			else
			{
				LabelTo( from, "a seed" );
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			LabelTo( from, "Choose a bowl of dirt to plant this seed in." );
		}
