						...
						case 0: // set price
						{
							...
							if ( m_From.AccessLevel == AccessLevel.Player && !m_Town.TaxChangeReady )
							{
								...
								if ( remaining.TotalMinutes < 4 )
									m_From.SendAsciiMessage( "You must wait a short while before changing prices again." );
								else if ( remaining.TotalMinutes < 10 )
									m_From.SendAsciiMessage( "You must wait several minutes before changing prices again." );
								else if ( remaining.TotalHours < 1 )
									m_From.SendAsciiMessage( "You must wait up to an hour before changing prices again." );
								else if ( remaining.TotalHours < 4 )
									m_From.SendAsciiMessage( "You must wait a few hours before changing prices again." );
								else 
									m_From.SendAsciiMessage( "You must wait several hours before changing prices again." );
							}
							else
							{
								m_Town.Tax = newTax;
