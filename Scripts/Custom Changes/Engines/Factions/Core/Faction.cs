	...
	public abstract class Faction : IComparable
	{
		...
		public void Broadcast( int hue, string text )
		{
			...
			for ( int i = 0; i < members.Count; ++i )
				members[i].Mobile.SendAsciiMessage( hue, text );
		}
		...
		{
			from.SendAsciiMessage( "Enter Faction Message:" );
		...
		{
			from.SendAsciiMessage( "Click on the player whom you wish to honor." );
		...
		public void HonorLeadership_OnTarget( Mobile from, object obj )
		{
			if ( obj is Mobile )
			{
				...
				{
					from.SendAsciiMessage( "Only faction mates can be honored this way." );
				}
				else if ( giveState.KillPoints < 5 ) // TODO: Verify 5 or 10
				{
					from.SendAsciiMessage( "You must have at least five kill points to honor them." );
				}
				else
				{
					...
					recv.SendAsciiMessage( "You have been honored with four kill points." );
				}
			}
			else
			{
				from.SendAsciiMessage( "You may only honor another player." );
			}
		}
		...
		{
			AddMember( mob );
			mob.SendAsciiMessage( "You have joined the faction." );
		}
		...
		public void OnJoinAccepted( Mobile mob )
		{
			...
			if ( pm.Young )
				pm.SendAsciiMessage( "You cannot join a faction as a young player." );
			else if ( pl != null && pl.IsLeaving )
				pm.SendAsciiMessage( "You cannot use the faction stone until you have finished quitting your current faction." );
			else if ( AlreadyHasCharInFaction( pm ) )
				pm.SendAsciiMessage( "You cannot join a faction because you already declared your allegiance with another character." );
			else if ( IsFactionBanned( mob ) )
				pm.SendAsciiMessage( "You are currently banned from the faction system." );
			else if ( pm.Guild != null )
			{
				Guild guild = pm.Guild as Guild;

				if ( guild.Leader != pm )
					pm.SendAsciiMessage( "You cannot join a faction because you are in a guild and not the guildmaster." );
				else if ( guild.Type != GuildType.Regular )
					pm.SendAsciiMessage( "You cannot join a faction because your guild is an Order or Chaos type." );
				else if ( guild.Enemies != null && guild.Enemies.Count > 0 )
					pm.SendAsciiMessage( "You cannot join a faction with active Wars." );
				else if ( !CanHandleInflux( guild.Members.Count ) )
					pm.SendAsciiMessage( "In the interest of faction stability, this faction declines to accept new members for now." );
				else
				{
		...
		public static void FactionItemReset_OnCommand( CommandEventArgs e )
		{
			...
			e.Mobile.SendAsciiMessage( "{0} items reset", count );
		}

		public static void FactionCommander_OnCommand( CommandEventArgs e )
		{
			e.Mobile.SendAsciiMessage( "Target a player to make them the faction commander." );
		...
		public static void FactionCommander_OnTarget( Mobile from, object obj )
		{
			if ( obj is PlayerMobile )
			{
				...
				if ( pl != null )
				{
					...
					from.SendAsciiMessage( "You have appointed them as the faction commander." );
				}
				else
				{
					from.SendAsciiMessage( "They are not in a faction." );
				}
			}
			else
			{
				from.SendAsciiMessage( "That is not a player." );
			}
		}

		public static void FactionElection_OnCommand( CommandEventArgs e )
		{
			e.Mobile.SendAsciiMessage( "Target a faction stone to open its election properties." );
		...
		public static void FactionElection_OnTarget( Mobile from, object obj )
		{
			if ( obj is FactionStone )
			{
				...
				if ( faction != null )
					...
					from.SendAsciiMessage( "That stone has no faction assigned." );
			}
			else
			{
				from.SendAsciiMessage( "That is not a faction stone." );
			}
		}

		public static void FactionKick_OnCommand( CommandEventArgs e )
		{
			e.Mobile.SendAsciiMessage( "Target a player to remove them from their faction." );
		...
		public static void FactionKick_OnTarget( Mobile from, object obj )
		{
			if ( obj is Mobile )
			{
				...
				if ( pl != null )
				{
					...
					mob.SendAsciiMessage( "You have been kicked from your faction." );
					from.SendAsciiMessage( "They have been kicked from their faction." );
				}
				else
				{
					from.SendAsciiMessage( "They are not in a faction." );
