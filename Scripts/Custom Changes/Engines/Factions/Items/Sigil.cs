...
namespace Server.Factions
{
	public class Sigil : BaseSystemController, IComparable
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			{
				LabelTo( from, "this sigil is not corrupted" ); // This sigil is not corrupted.
			}

			if ( IsCorrupting )
				LabelTo( from, "this sigil is in the process of being corrupted" ); // This sigil is in the process of being corrupted.
			else if ( IsPurifying )
				LabelTo( from, "this sigil has been recently corrupted, and is undergoing the process of purification" ); // This sigil has been recently corrupted, and is undergoing purification.
			else
				LabelTo( from, "this sigil is not in the process of being corrupted" ); // This sigil is not in the process of being corrupted.
		}

		public override bool CheckLift( Mobile from, Item item )
		{
			from.SendAsciiMessage( "You must use the stealing skill to pick up the sigil" );
			return false;
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsChildOf( from.Backpack ) )
			{
				...
				from.SendAsciiMessage( "Click on a sigil monolith or player" );
			}
		}
		...
		private void Sigil_OnTarget( Mobile from, object obj )
		{
			...
			if ( obj is Mobile )
			{
				if ( obj is PlayerMobile )
				{
					...
					if ( toFaction == null )
						from.SendAsciiMessage( "You cannot give the sigil to someone not in a faction." );
					else if ( fromFaction != toFaction )
						from.SendAsciiMessage( "You cannot give the sigil to someone not in your faction." );
					else if ( Sigil.ExistsOn( targ ) )
						from.SendAsciiMessage( "You cannot give this sigil to someone who already has a sigil." );
					else if( !targ.Alive )
						from.SendAsciiMessage( "You cannot give a sigil to a dead person." );
				...
				{
					from.SendAsciiMessage( "You cannot give the sigil to them." );
				}
			}
			#endregion
			else if ( obj is BaseMonolith  )
			{
				...
				if ( obj is StrongholdMonolith )
				{
					...
					if ( m.Faction == null || m.Faction != Faction.Find( from ) )
						from.SendAsciiMessage( "You can't place that on an enemy monolith." );
					else if ( m.Town == null || m.Town != m_Town )
						from.SendAsciiMessage( "That is not the correct faction monolith" );
					else
					{
						m.Sigil = this;
				...
				else if ( obj is TownMonolith )
				{
					...
					if ( m.Town == null || m.Town != m_Town )
						from.SendAsciiMessage( "This is not the correct town sigil monolith." );
					else if ( m_Corrupted == null || m_Corrupted != Faction.Find( from ) )
						from.SendAsciiMessage( "Your faction did not corrupt this sigil.  Take it to your stronghold." );
					else
					{
						m.Sigil = this;
			...
			{
				from.SendAsciiMessage( "You can't use the sigil on that." ); 
			}
