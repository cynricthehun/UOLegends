...
namespace Server.Spells
{
	public abstract class Spell : ISpell
	{
		...
		public virtual bool CheckSpellbook{ get{ return true; } }
		...
		public virtual bool OnCasterMoving( Direction d )
		{
			...
			{
				m_Caster.SendAsciiMessage( "You are frozen and can not move." );
				return false;
			}
		...
		public virtual double GetInscribeSkill( Mobile m )
		{
			...
			// m.CheckSkill( SkillName.Inscribe, 0.0, 100.0 );
		...
		{
			if ( message )
				m_Caster.SendAsciiMessage( "Your concentration is disturbed, thus ruining thy spell." );
		}
		...
		public virtual void SayMantra()
		{
			...
			if ( m_Info.Mantra != null && m_Info.Mantra.Length > 0 && ( m_Caster.Player || Caster.Body.IsHuman ) )
		...
		public virtual bool CheckSequence()
		{
			...
			{
				m_Caster.SendAsciiMessage( "You cannot cast a spell while frozen." );
				DoFizzle();
			}
			else if ( CheckFizzle() )
			{
				...
				if ( TransformationSpell.UnderTransformation( m_Caster, typeof( VampiricEmbraceSpell ) ) )
				{
					...
					if ( garlic )
					{
						m_Caster.SendAsciiMessage( "The garlic burns you!" );
