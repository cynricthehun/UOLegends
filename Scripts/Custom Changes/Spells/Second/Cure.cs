...
namespace Server.Spells.Second
{
	public class CureSpell : Spell
	{
			...
			else if ( CheckBSequence( m ) )
			{
				...
				if ( p != null )
				{
					...
					if ( chanceToCure > Utility.Random( 100 ) )
					{
						...
						{
							if ( Caster != m )
								Caster.SendAsciiMessage( "You have cured the target of all poisons!" );

							m.SendAsciiMessage( "You have been cured of all poisons." );
						}
					}
					else
					{
						m.SendAsciiMessage( "You have failed to cure your target!" );
					}
				}
		...
		{
			return base.GetCastDelay() + TimeSpan.FromSeconds( 1.0 );
		}
	}
}
