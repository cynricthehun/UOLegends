...
namespace Server.Spells.Seventh
{
	public class EnergyFieldSpell : Spell
	{
		...
		private class InternalItem : Item
		{
			...
			{
				if ( m_Caster != null && (!Core.AOS || m != m_Caster) && SpellHelper.ValidIndirectTarget( m_Caster, m ) && m_Caster.CanBeHarmful( m, false ) )
				{
					m_Caster.DoHarmful( m );

					int damage = 0;

					if ( !Core.AOS && m.CheckSkill( SkillName.MagicResist, 35.0, 85.0 ) )
					{
						damage = 0;
		...
		{
			return base.GetCastDelay() + TimeSpan.FromSeconds( 3.5 );
		}
	}
}
