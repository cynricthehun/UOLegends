...
namespace Server.Items
{
	public abstract class BaseOre : Item, ICommodity
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendAsciiMessage( "Select the forge on which to smelt the ore, or another pile of ore with which to combine it." );
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( IsForge( targeted ) )
				{
					...
					{
						from.SendAsciiMessage( "You have no idea how to smelt this strange ore!" );
						return;
					}
					...
					if ( from.CheckTargetSkill( SkillName.Mining, targeted, minSkill, maxSkill ) )
					{
						...
						else
						{
							...
							from.SendAsciiMessage( "You smelt the ore removing the impurities and put the metal in your backpack." );
						}
					}
					else if ( m_Ore.Amount < 2 )
					{
						from.SendAsciiMessage( "You burn away the impurities but are left with no useable metal." );
						m_Ore.Delete();
					}
					else
					{
						from.SendAsciiMessage( "You burn away the impurities but are left with less useable metal." );
						m_Ore.Amount /= 2;
					}
				}
			}
	...
	public class IronOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " iron ore" );
				}
				else
				{
	...
	public class DullCopperOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " dull copper ore" );
				}
				else
				{
					LabelTo( from, "dull copper ore" );	
				}
			}
			else
			{
	...
	public class ShadowIronOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " shadow iron ore" );
				}
				else
				{
					LabelTo( from, "shadow iron ore" );	
				}
			}
			else
			{
	...
	public class CopperOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " copper ore" );
				}
				else
				{
					LabelTo( from, "copper ore" );	
				}
			}
			else
			{
	...
	public class BronzeOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " bronze ore" );
				}
				else
				{
					LabelTo( from, "bronze ore" );	
				}
			}
			else
			{
	...
	public class GoldOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " gold ore" );
				}
				else
				{
					LabelTo( from, "gold ore" );	
				}
			}
			else
			{
	...
	public class AgapiteOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " agapite ore" );
				}
				else
				{
					LabelTo( from, "agapite ore" );	
				}
			}
			else
			{
	...
	public class VeriteOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " verite ore" );
				}
				else
				{
					LabelTo( from, "verite ore" );	
				}
			}
			else
			{
	...
	public class ValoriteOre : BaseOre
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " valorite ore" );
				}
				else
				{
					LabelTo( from, "valorite ore" );	
				}
			}
			else
			{
