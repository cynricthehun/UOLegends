	...
	public class HolidayTree : Item
	{
		...
		private class TreeTrunk : Item
		{
			...
			public override void OnSingleClick( Mobile from )
			{
				if ( this.Name == null )
				{
					LabelTo( from, "a tree for the holidays" );
				}
				else
				{
					LabelTo( from, this.Name );
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( this.GetWorldLocation(), 1 ) )
			{
				if ( m_Placer == null || from == m_Placer || from.AccessLevel >= AccessLevel.GameMaster )
				{
					...
					from.SendAsciiMessage( "A deed for the tree has been placed in your backpack." );
				}
				else
				{
					from.SendAsciiMessage( "You cannot take this tree down." );
				}
			}
			else
			{
