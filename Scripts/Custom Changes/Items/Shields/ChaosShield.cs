	...
	public class ChaosShield : BaseShield
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name  == null )
			{
				if ( this.Identified == true || ( this.Durability == ArmorDurabilityLevel.Regular && this.ProtectionLevel == ArmorProtectionLevel.Regular ) )
				{
					...
					else if ( this.Quality == ArmorQuality.Exceptional )
					{
						if ( this.LootType != LootType.Blessed )
						{
							...
							{
								LabelTo( from, "an exceptional chaos shield" );
							}
							else
							{
								LabelTo( from, "an exceptional chaos shield (crafted by:" + this.Crafter.Name + ")" );
							}
						}
						if ( this.LootType == LootType.Blessed )
						{
							if ( this.Crafter == null )
							{
								LabelTo( from, "a blessed, exceptional chaos shield" );
							}
							else
							{
								LabelTo( from, "a blessed, exceptional chaos shield (crafted by:" + this.Crafter.Name + ")" );
							}
						}
					}
				}
				...
				{
					LabelTo( from, "a magic chaos shield" );
				}
