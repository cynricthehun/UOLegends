...
namespace Server.Items
{
	public class HolidayTreeDeed : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a deed for a holiday tree");
			}
			else
			{
		...
		public bool ValidatePlacement( Mobile from, Point3D loc )
		{
			...
			{
				from.SendAsciiMessage( "You will have to wait till next December to put your tree back up for display." );
				return false;
			}
			...
			{
				from.SendAsciiMessage( "The holiday tree can only be placed in your house." );
				return false;
			}

			if ( !map.CanFit( loc, 20 ) )
			{
				from.SendAsciiMessage( "You cannot build that there." );
				return false;
			}
