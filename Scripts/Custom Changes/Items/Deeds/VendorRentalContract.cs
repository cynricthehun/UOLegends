...
namespace Server.Items
{
	public class VendorRentalContract : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a vendor rental contract");
			}
			else
			{
		...
		public bool IsUsableBy( Mobile from, bool byLandlord, bool byBackpack, bool noOfferee, bool sendMessage )
		{
			...
			{
				if ( sendMessage )
					from.SendAsciiMessage( "That item is currently in use." );
			...
			if ( byLandlord && IsLandlord( from ) )
			{
				...
				{
					if ( sendMessage )
						from.SendAsciiMessage( "Target is too far away." );
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else if ( !IsLockedDown )
			{
				...
				{
					from.SendAsciiMessage( "This item must be in your backpack to be used." );
					return;
				}
				...
				{
					from.SendAsciiMessage( "You must be standing inside of a house that you own to make use of this contract." );
				}
				else if ( !house.IsAosRules )
				{
					from.SendAsciiMessage( "Rental contracts can only be placed in AOS-enabled houses." );
				}
				else if ( !house.Public )
				{
					from.SendAsciiMessage( "Rental contracts can only be placed in public houses." );
				}
				else if ( !house.CanPlaceNewVendor() )
				{
					from.SendAsciiMessage( "You do not have enought storage available to place this contract." );
				}
				else
				{
					from.SendAsciiMessage( "Target the exact location you wish to rent out." );
		...
		private class RentTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( house == null || !house.IsOwner( from ) )
				{
					from.SendAsciiMessage( "The location being rented out must be inside of your house." );
				}
				else if ( BaseHouse.FindHouseAt( from ) != house )
				{
					from.SendAsciiMessage( "You must be located inside of the house in which you are trying to place the contract." );
				}
				else if ( !house.IsAosRules )
				{
				...
				{
					from.SendAsciiMessage( "You cannot place a contract in a condemned house." );
				}
				else if ( !house.CanPlaceNewVendor() )
				{
					from.SendAsciiMessage( "You do not have enought storage available to place this contract." );
				...
				{
					from.SendAsciiMessage( "A vendor cannot exist at that location.  Please try again." );
				}
				else
				{
					...
					if ( vendor )
					{
						from.SendAsciiMessage( "You may not place a rental contract at this location while other beings occupy it." );
					}
					else if ( contract )
					{
						from.SendAsciiMessage( "That location is cluttered. Please clear out any objects there and try again." );
					}
					else
					{
			...
			{
				from.SendAsciiMessage( "You decide not to place the contract at this time." );
			}
		}
