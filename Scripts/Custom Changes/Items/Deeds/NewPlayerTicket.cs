...
namespace Server.Items
{
	public class NewPlayerTicket : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a young player ticket");
			}
			else
			{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from != m_Owner )
			{
				from.SendAsciiMessage( "This isn't your ticket! Shame on you! You have to use YOUR ticket." );
			}
			...
			{
				from.SendAsciiMessage( "Target any other ticket marked NEW PLAYER to win a prize." );
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				{
					from.SendAsciiMessage( "You can't target the same ticket!" );
				}
				else if ( targeted is NewPlayerTicket )
				{
					...
					{
						from.SendAsciiMessage( "That is not a valid ticket." );
					}
					else
					{
				...
				{
					from.SendAsciiMessage( "You need to find another ticket marked NEW PLAYER." );
				}
				else
				{
					from.SendAsciiMessage( "You will need to select a ticket." );
				}
			}
		}
		...
		private class InternalGump : Gump
		{
			...
			public InternalGump( Mobile from, NewPlayerTicket ticket ) : base( 50, 50 )
			{
				...
				AddHtml( 80, 270, 320, 35, "A piece of ranger armor", false, false ); // Ranger Armor
