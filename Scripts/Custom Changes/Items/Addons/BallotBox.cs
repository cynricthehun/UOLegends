...
namespace Server.Items
{
	public class BallotBox : AddonComponent
	{
		...
		{
			SendAsciiMessageTo( from, "I'm a ballot box, not a container!" );
			return false;
		}
				...
				switch ( info.ButtonID )
				{
					case 1: // change topic
					{
						if ( isOwner )
						{
							...
							from.SendAsciiMessage( 0x35, "Enter a line of text for your ballot, and hit ENTER. Hit ESC after the last line is entered." );
					...
					case 2: // reset votes
					{
						...
						{
							m_Box.ClearVotes();
							from.SendAsciiMessage( "Votes zeroed out." );
						}
					...
					case 3: // aye
					{
						if ( !isOwner )
						{
							...
							{
								from.SendAsciiMessage( "You have already voted on this ballot." );
							}
							else
							{
								m_Box.Yes.Add( from );
								from.SendAsciiMessage( "Your vote has been registered." );
							}
						}
		...
		private class TopicPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				{
					from.SendAsciiMessage( 0x35, "Next line or ESC to finish:" );
					from.Prompt = new TopicPrompt( m_Box );
				}
				else
				{
					from.SendAsciiMessage( 0x35, "Ballot entry complete." );
	...
	public class BallotBoxDeed : BaseAddonDeed
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a ballot box deed" );
			}
			else
			{
				LabelTo( from, this.Name );
