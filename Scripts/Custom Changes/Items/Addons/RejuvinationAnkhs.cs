	...
	public class RejuvinationAddonComponent : AddonComponent
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.BeginAction( typeof( RejuvinationAddonComponent ) ) )
			{
				...
				if ( random == 1 || random == 4 )
				{
					...
					SendAsciiMessageTo( from, "A sense of warmth fills your body!" ); 
				}

				if ( random == 2 || random == 4 )
				{
					from.Mana = from.ManaMax;
					SendAsciiMessageTo( from, "A feeling of power surges through your veins!" );
				}

				if ( random == 3 || random == 4 )
				{
					from.Stam = from.StamMax;
					SendAsciiMessageTo( from, "You feel as though you've slept for days!" );
				}
		...
		public virtual void ReleaseUseLock_Callback( object state )
		{
			...
			if ( random == 4 )
			{
				...
				from.Mana = from.ManaMax;
				from.Stam = from.StamMax;
				SendAsciiMessageTo( from, "You feel completely rejuvinated!" );
			}
		}
			...
			if ( m.Player && Utility.InRange( Location, m.Location, 3 ) && !Utility.InRange( Location, oldLocation, 3 ) )
			{
				if ( DateTime.Now >= m_NextMessage )
				{
					if ( Components.Count > 0 )
						((AddonComponent)Components[0]).SendAsciiMessageTo( m, "An overwhelming sense of peace fills you." );
