...
namespace Server.Items
{
	...
	public class PickpocketDip : AddonComponent
	{
		...
		public void Use( Mobile from )
		{
			...
			{
				SendAsciiMessageTo( from, "You successfully avoid disturbing the dip while searching it." );
			}
			else
			{
				...
				BeginSwing();
				ProcessDelta();
				SendAsciiMessageTo( from, "You carelessly bump the dip and start it swinging." );
			}
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( !from.InRange( GetWorldLocation(), 1 ) )
				SendAsciiMessageTo( from, "You are too far away to do that." );
			else if ( Swinging )
				SendAsciiMessageTo( from, "You have to wait until it stops swinging." );
			else if ( from.Skills[SkillName.Stealing].Base >= m_MaxSkill )
				SendAsciiMessageTo( from, "Your ability to steal cannot improve any further by simply practicing on a dummy." );
			else if ( from.Mounted )
				SendAsciiMessageTo( from, "You can't practice on this while on a mount." );
			else
				Use( from );
		}
