...
namespace Server.Items
{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( Content == BeverageType.Ale )
					LabelTo( from, "a bottle of ale" );
				if ( Content == BeverageType.Cider )
					LabelTo( from, "a bottle of cider" );
				if ( Content == BeverageType.Liquor )
					LabelTo( from, "a bottle of liquor" );
				if ( Content == BeverageType.Milk )
					LabelTo( from, "a bottle of milk" );
				if ( Content == BeverageType.Wine )
					LabelTo( from, "a bottle of wine" );
				if ( Content == BeverageType.Water )
					LabelTo( from, "a bottle of water" );
			}
			else
			{
	...
	public class Jug : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			{
				if ( Content == BeverageType.Ale )
					LabelTo( from, "a jug of ale" );
				if ( Content == BeverageType.Cider )
					LabelTo( from, "a jug of cider" );
				if ( Content == BeverageType.Liquor )
					LabelTo( from, "a jug of liquor" );
				if ( Content == BeverageType.Milk )
					LabelTo( from, "a jug of milk" );
				if ( Content == BeverageType.Wine )
					LabelTo( from, "a jug of wine" );
				if ( Content == BeverageType.Water )
					LabelTo( from, "a jug of water" );
			}
			else
			{
				LabelTo( from, this.Name );
	...
	public class CeramicMug : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				...
				{
					if ( Content == BeverageType.Ale )
						LabelTo( from, "a ceramic mug of ale" );
					if ( Content == BeverageType.Cider )
						LabelTo( from, "a ceramic mug of cider" );
					if ( Content == BeverageType.Liquor )
						LabelTo( from, "a ceramic mug of liquor" );
					if ( Content == BeverageType.Milk )
						LabelTo( from, "a ceramic mug of milk" );
					if ( Content == BeverageType.Wine )
						LabelTo( from, "a ceramic mug of wine" );
					if ( Content == BeverageType.Water )
						LabelTo( from, "a ceramic mug of water" );
				}
				else
				{
					LabelTo( from, "a ceramic mug" );
				}
			}
			else
			{
	...
	public class PewterMug : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				...
				{
					if ( Content == BeverageType.Ale )
						LabelTo( from, "a pewter mug of ale" );
					if ( Content == BeverageType.Cider )
						LabelTo( from, "a pewter mug of cider" );
					if ( Content == BeverageType.Liquor )
						LabelTo( from, "a pewter mug of liquor" );
					if ( Content == BeverageType.Milk )
						LabelTo( from, "a pewter mug of milk" );
					if ( Content == BeverageType.Wine )
						LabelTo( from, "a pewter mug of wine" );
					if ( Content == BeverageType.Water )
						LabelTo( from, "a pewter mug of water" );
				}
				else
				{
					LabelTo( from, "a pewter mug" );
				}
			}
			else
			{
	...
	public class Goblet : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				...
				{
					if ( Content == BeverageType.Ale )
						LabelTo( from, "a goblet of ale" );
					if ( Content == BeverageType.Cider )
						LabelTo( from, "a goblet of cider" );
					if ( Content == BeverageType.Liquor )
						LabelTo( from, "a goblet of liquor" );
					if ( Content == BeverageType.Milk )
						LabelTo( from, "a goblet of milk" );
					if ( Content == BeverageType.Wine )
						LabelTo( from, "a goblet of wine" );
					if ( Content == BeverageType.Water )
						LabelTo( from, "a goblet of water" );
				}
				else
				{
	...
	public class GlassMug : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				...
				{
					if ( Content == BeverageType.Ale )
						LabelTo( from, "a glass mug of ale" );
					if ( Content == BeverageType.Cider )
						LabelTo( from, "a glass mug of cider" );
					if ( Content == BeverageType.Liquor )
						LabelTo( from, "a glass mug of liquor" );
					if ( Content == BeverageType.Milk )
						LabelTo( from, "a glass mug of milk" );
					if ( Content == BeverageType.Wine )
						LabelTo( from, "a glass mug of wine" );
					if ( Content == BeverageType.Water )
						LabelTo( from, "a glass mug of water" );
				}
				else
				{
	...
	public class Pitcher : BaseBeverage
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				...
				{
					if ( Content == BeverageType.Ale )
						LabelTo( from, "a pitcher of ale" );
					if ( Content == BeverageType.Cider )
						LabelTo( from, "a pitcher of cider" );
					if ( Content == BeverageType.Liquor )
						LabelTo( from, "a pitcher of liquor" );
					if ( Content == BeverageType.Milk )
						LabelTo( from, "a pitcher of milk" );
					if ( Content == BeverageType.Wine )
	...
	public abstract class BaseBeverage : Item, IHasQuantity
	{
		...
		public virtual void Fill_OnTarget( Mobile from, object targ )
		{
			...
			else if ( targ is Item )
			{
				...
				from.SendAsciiMessage( "You fill the container with water." );
			}
					...
					if ( qs is WitchApprenticeQuest )
					{
						...
						if ( obj != null && !obj.Completed && obj.Ingredient == Ingredient.SwampWater )
						{
							...
							if ( contains )
							{
								...
								player.SendAsciiMessage( "You dip the container into the disgusting swamp water, collecting enough for the Hag's vile stew." );
								obj.Complete();
							}
						}
					}
		...
		public static void CheckHeaveTimer( Mobile from )
		{
			...
			else
			{
				...
				if ( t != null )
				{
					...
					from.SendAsciiMessage( "You feel sober." );
				}
			}
		}
			...
			protected override void OnTick()
			{
				...
				else if ( m_Drunk.Alive )
				{
					...
					if ( m_Drunk.BAC <= 0 )
					{
						...
						m_Drunk.SendAsciiMessage( "You feel sober." );
					}
				}
			}
		}
		...
		public virtual void Pour_OnTarget( Mobile from, object targ )
		{
			...
			if ( targ is BaseBeverage )
			{
				...
				{
					from.SendAsciiMessage( "Couldn't pour it there. It was already full." );
				}
				else if ( !bev.IsEmpty )
				{
					from.SendAsciiMessage( "Can't pour it there." );
				}
				else
				{
