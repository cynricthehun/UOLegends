			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is TribalBerry )
				{
					if ( from.Skills[SkillName.Cooking].Base >= 80.0 )
					{
						...
						from.SendAsciiMessage( "You combine the berry and the flour into the tribal paint worn by the savages." );
					}
					else
					{
						from.SendAsciiMessage( "You don't have the cooking skill to create the body paint." );
					}
				}
			}
		}
