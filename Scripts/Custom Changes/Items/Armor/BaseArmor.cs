...
namespace Server.Items
{
	public abstract class BaseArmor : Item, IScissorable, IFactionItem, ICraftable
	{
		...
		public bool Scissor( Mobile from, Scissors scissors )
		{
			...
			{
				from.SendAsciiMessage( "Items you wish to cut must be in your backpack." );
				return false;
			}
				...
				if ( item is BaseArmor )
				{
					...
					if ( !armor.AllowMaleWearer && m.Body.IsMale && m.AccessLevel < AccessLevel.GameMaster )
					{
						if ( armor.AllowFemaleWearer )
							m.SendAsciiMessage( "Only females can wear this." );
						else
		...
		public override bool CanEquip( Mobile from )
		{
			if ( !AllowMaleWearer && from.Body.IsMale && from.AccessLevel < AccessLevel.GameMaster )
			{
				if ( AllowFemaleWearer )
					from.SendAsciiMessage( "Only females can wear this." );
				else
			...
			else
			{
				...
				{
					from.SendAsciiMessage( "You do not have enough dexterity to equip this item." );
					return false;
				} 
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( m_Durability != ArmorDurabilityLevel.Regular )
			{
				if ( m_Durability == ArmorDurabilityLevel.Durable )
					m_dur = "durable";
				if ( m_Durability == ArmorDurabilityLevel.Substantial )
					m_dur = "substantial";
				if ( m_Durability == ArmorDurabilityLevel.Massive )
					m_dur = "massive";
				if ( m_Durability == ArmorDurabilityLevel.Fortified )
					m_dur = "fortified";
				if ( m_Durability == ArmorDurabilityLevel.Indestructible )
					m_dur = "indestructible";
			}
			if ( m_Protection != ArmorProtectionLevel.Regular )
			{
				if ( m_Protection == ArmorProtectionLevel.Defense )
					m_pro = "defense";
				if ( m_Protection == ArmorProtectionLevel.Guarding )
					m_pro = "guarding";
				if ( m_Protection == ArmorProtectionLevel.Hardening )
					m_pro = "hardening";
				if ( m_Protection == ArmorProtectionLevel.Fortification )
					m_pro = "fortification";
				if ( m_Protection == ArmorProtectionLevel.Invulnerability )
					m_pro = "invulnerability";
			}
			if ( m_FactionState != null )
			{
			...
			if ( m_Durability != ArmorDurabilityLevel.Regular || m_Protection != ArmorProtectionLevel.Regular )
			{
				if ( this.Identified == true )
				{
					if ( m_Durability != ArmorDurabilityLevel.Regular && m_Protection == ArmorProtectionLevel.Regular )
					{
						from.Send( new AsciiMessage( this.Serial, 0, MessageType.Label, 0x3B2, 3, null, "(" + m_dur + ")" ) );
					}
					else if ( m_Durability == ArmorDurabilityLevel.Regular && m_Protection != ArmorProtectionLevel.Regular )
					{
						from.Send( new AsciiMessage( this.Serial, 0, MessageType.Label, 0x3B2, 3, null, "(" + m_pro + ")" ) );
					}
					else if ( m_Durability != ArmorDurabilityLevel.Regular && m_Protection != ArmorProtectionLevel.Regular )
					{
						from.Send( new AsciiMessage( this.Serial, 0, MessageType.Label, 0x3B2, 3, null, "(" + m_dur + "/" + m_pro + ")" ) );
					}
				}
			}
		}
