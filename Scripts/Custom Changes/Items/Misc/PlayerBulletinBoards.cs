	...
	public abstract class BasePlayerBB : Item, ISecurable
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( house == null || !house.IsLockedDown( this ) )
				from.SendAsciiMessage( "This bulletin board must be locked down in a house to be usable." );
		...
		public class PostPrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				{
					from.SendAsciiMessage( "You are not allowed to post to this bulletin board." );
					return;
				}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				case 1: // Post message
				{
					...
					from.SendAsciiMessage( "Please enter your message:" );
				...
				case 2: // Set title
				{
					if ( house.IsOwner( from ) )
					{
						...
						from.SendAsciiMessage( "Enter new title:" );
					}
				...
				case 3: // Post greeting
				{
					if ( house.IsOwner( from ) )
					{
						...
						from.SendAsciiMessage( "Enter new greeting (this will always be the first post):" );
					}
				...
				case 6: // Banish poster
				{
					if ( house.IsOwner( from ) )
					{
						if ( page >= 1 && page <= board.Messages.Count )
						{
							...
							{
								from.SendAsciiMessage( "Uh oh...a bigger boot may be required." );
							}
							else if ( house.IsFriend( poster ) )
							{
								from.SendAsciiMessage( "That person is a friend, co-owner, or owner of this house, and therefore cannot be banished!" );
							}
							else if ( poster is PlayerVendor )
							{
								from.SendAsciiMessage( "You cannot eject a vendor." );
							}
							else if ( house.Bans.Count >= BaseHouse.MaxBans )
							{
								from.SendAsciiMessage( "The ban limit for this house has been reached!" );
							}
							else if ( house.IsBanned( poster ) )
							{
								from.SendAsciiMessage( "This person is already banned!" );
							}
							else if ( poster is BaseCreature && ((BaseCreature)poster).NoHouseRestrictions )
							{
								from.SendAsciiMessage( "You cannot ban that." );
							}
							else
							{
								...
								from.SendAsciiMessage( "That person has been banned from this house." );
