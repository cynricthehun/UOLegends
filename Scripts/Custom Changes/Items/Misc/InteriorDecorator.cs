...
namespace Server.Items
{
	...
	public class InteriorDecorator : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "an interior decorator" );
			}
			else
			{
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is Item && InteriorDecorator.CheckUse( m_Decorator, from ) )
				{
					...
					{
						from.SendAsciiMessage( "That is not in your house." );
					}
					else if ( !house.IsLockedDown( item ) && !house.IsSecure( item ) )
					{
						from.SendAsciiMessage( "That is not locked down." );
					}
					else if ( item is VendorRentalContract )
					{
						from.SendAsciiMessage( "You cannot use the house decorator on that object." );
					}
					else if ( item.TotalWeight + item.PileWeight > 100 )
					{
						from.SendAsciiMessage( "That is too heavy." );
					}
					else
					{
			...
			private static void Turn( Item item, Mobile from )
			{
				...
				if( attributes.Length > 0 )
					...
					from.SendAsciiMessage( "You cannot turn that." );
			}
			...
			private static void Up( Item item, Mobile from )
			{
				...
				if ( floorZ > int.MinValue && item.Z < (floorZ + 15) ) // Confirmed : no height checks here
					...
					from.SendAsciiMessage( "You cannot raise it up any higher." );
			}
			...
			private static void Down( Item item, Mobile from )
			{
				...
				if ( floorZ > int.MinValue && item.Z > GetFloorZ( item ) )
					...
					from.SendAsciiMessage( "You cannot lower it down any further." );
			}
