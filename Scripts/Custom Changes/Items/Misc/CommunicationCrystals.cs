...
namespace Server.Items
{
	...
	public class BroadcastCrystal : Item
	{
		...
		{
			LabelTo( from, "a communication crystal" );

			LabelTo( from, this.Active ? "active" : "inactive" );
			LabelTo( from, "broadcast" );
			LabelTo( from, "charges: {0}", this.Charges.ToString() );

			if ( Receivers.Count > 0 )
				LabelTo( from, "links: {0}", Receivers.Count.ToString() );
		}
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is ReceiverCrystal )
				{
					...
					{
						from.SendAsciiMessage( "This crystal is already linked with that crystal." );
					}
					else if ( receiver.Sender != null )
					{
						from.SendAsciiMessage( "That receiver crystal is already linked to another broadcast crystal." );
					}
					else
					{
				...
				else if ( targeted == from )
				{
					...
					from.SendAsciiMessage( "You unlink the broadcast crystal from all of its receivers." );
				}
				else
				{
	...
	public class ReceiverCrystal : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			...
			LabelTo( from, this.Active ? "active" : "inactive" ); // active / inactive
			LabelTo( from, "reciever" ); // receiver
		}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted == from )
				{
					if ( m_Crystal.Sender != null )
					{
						...
						from.SendAsciiMessage( "You unlink the receiver crystal." );
					}
					else
					{
						from.SendAsciiMessage( "That receiver crystal is not linked." );
					}
				}
				else
				{
					...
					if ( targItem != null && targItem.VerifyMove( from ) )
					{
						...
						if ( info != null )
						{
							from.SendAsciiMessage( "This crystal cannot be recharged." );
							return;
						}
					}
