...
namespace Server.Items
{
	public class PublicMoongate : Item
	{
		...
		public bool UseGate( Mobile m )
		{
			if ( m.Criminal )
			{
				m.SendAsciiMessage( 0x22, "Thou'rt a criminal and cannot escape so easily." );
				return false;
			}
			else if ( m.Spell != null )
			{
				m.SendAsciiMessage( "You are too busy to do that at the moment." );
				return false;
			}
			else
			{
	...
	public class MoongateGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			{
				m_Mobile.SendAsciiMessage( "You are too far away to use the gate." );
			}
			else if ( m_Mobile.Player && m_Mobile.Kills >= 5 && list.Map != Map.Felucca )
			{
				m_Mobile.SendAsciiMessage( "You are not allowed to travel there." );
			}
			...
			{
				m_Mobile.SendAsciiMessage( 0x22, "Thou'rt a criminal and cannot escape so easily." );
			}
			else if ( m_Mobile.Spell != null )
			{
				m_Mobile.SendAsciiMessage( "You are too busy to do that at the moment." );
			}
			else if ( m_Mobile.Map == list.Map && m_Mobile.InRange( entry.Location, 1 ) )
			{
				m_Mobile.SendAsciiMessage( "You are already there." );
			}
			else
			{
