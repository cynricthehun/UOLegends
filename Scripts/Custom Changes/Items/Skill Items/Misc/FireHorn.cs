...
namespace Server.Items
{
	public class FireHorn : Item
	{
		...
		private bool CheckUse( Mobile from )
		{
			...
			{
				from.SendAsciiMessage( "You must take a moment to catch your breath." ); // 
				return false;
			}
			...
			{
				from.SendAsciiMessage( "You do not have enough sulfurous ash." ); // 
				return false;
			}
		...
		public void Use( Mobile from, IPoint3D loc )
		{
			...
			{
				from.SendAsciiMessage( "The horn emits a pathetic squeak." ); // 
			...
			{
				from.SendAsciiMessage( "The fire horn crumbles in your hands." ); // 
				this.Delete();
			}
		}
		...
		private static void EndAction( object state )
		{
			...
			m.SendAsciiMessage( "You catch your breath." ); // 
		}
