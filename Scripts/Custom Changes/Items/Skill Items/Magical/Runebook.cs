	...
	public class Runebook : Item, ISecurable, ICraftable
	{
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			if ( dropped is RecallRune )
			{
				if ( !CheckAccess( from ) )
				{
					from.SendAsciiMessage( "That cannot be done while the book is locked down." );
				}
				else if ( IsOpen( from ) )
				{
					from.SendAsciiMessage( "You cannot place objects in the book while viewing the contents." );
				}
				else if ( m_Entries.Count < 16 )
				{
					...
					{
						from.SendAsciiMessage( "This rune does not have a marked location." );
					}
				}
				else
				{
					from.SendAsciiMessage( "This runebook is full." );
				}
			}
			else if ( dropped is RecallScroll )
			{
				...
				{
					from.SendAsciiMessage( "This book already has the maximum amount of charges." );
				}
			}
