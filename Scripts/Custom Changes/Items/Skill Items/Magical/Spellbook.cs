	...
	public class Spellbook : Item
	{
		...
		public override bool OnDragDrop( Mobile from, Item dropped )
		{
			if ( dropped is SpellScroll && dropped.Amount == 1 )
			{
				...
				{
					from.SendAsciiMessage( "That spell is already present in that spellbook." ); // 
					return false;
				}
				else
				{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a spellbook" );
			}
			else
			{
				LabelTo( from, this.Name );
			}
			LabelTo( from, this.SpellCount > 0 ? m_Count.ToString() + " spells" : "" );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( Parent == from || ( pack != null && Parent == pack ) )
				...
				from.SendAsciiMessage( "The spellbook must be in your backpack (and not in a container within) to open." );
		}
