	...
	public class TaxidermyKit : Item
	{
		...
		private class CorpseTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				{
					from.SendAsciiMessage( "That is not a corpse!" ); // That is not a corpse!
				}
				else if ( targeted is Corpse && ((Corpse)targeted).VisitedByTaxidermist )
				{
					from.SendAsciiMessage( "That corpse seems to have been visited by a taxidermist already." ); //
				}
				...
				{
					from.SendAsciiMessage( "You would not understand how to use the kit." ); // 
				}
				else
				{
					object obj = targeted;
					...
					for ( int i = 0; obj != null && i < m_Table.GetLength( 0 ); ++i )
					{
						if ( m_Table[i, 0] == obj.GetType() )
						{
							...
							{
								from.SendAsciiMessage( "You review the corpse and find it worthy of a trophy." );
								from.SendAsciiMessage( "You use your kit up making the trophy." ); 
							...
							{
								from.SendAsciiMessage( "You do not have enough boards." );
								return;
							}
						}
					}

					from.SendAsciiMessage( "That does not look like something you want hanging on a wall." ); // 
				}
			}
		}
	}
			...
			if ( IsChildOf( from.Backpack ) )
			{
				...
				if ( house != null && house.IsCoOwner( from ) )
				{
					...
					if ( northWall )
						...
						from.SendAsciiMessage( "The trophy must be placed next to a wall." ); // 
