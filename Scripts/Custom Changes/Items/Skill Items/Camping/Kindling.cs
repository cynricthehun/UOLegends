	...
	public class Kindling : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				if ( this.Amount > 1 )
				{
					LabelTo( from, this.Amount + " kindling" );
				}
				else
				{
					LabelTo( from, "kindling" );
				}
			}
			else
			{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.SendAsciiMessage( "There is not a spot nearby to place your campfire." ); // 
			}
			else if ( !from.CheckSkill( SkillName.Camping, 0.0, 100.0 ) )
			{
				from.SendAsciiMessage( "You fail to ignite the campfire." ); // 
			}
			else
			{
				Consume();
