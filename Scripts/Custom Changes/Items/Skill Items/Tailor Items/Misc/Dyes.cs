...
namespace Server.Items
{
	public class Dyes : Item
	{
		...
		{
			from.SendAsciiMessage( "Select the dye tub to use the dyes on." );
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is DyeTub )
				{
					...
					{
						from.SendAsciiMessage( "You can not use this on a black dye tub." );
					}
					else
					{
				...
				{
					from.SendAsciiMessage( "Use this on a dye tub." );
				}
			}
		}
	}
