			...
			if ( player != null )
			{
				...
				if ( qs is WitchApprenticeQuest )
				{
					...
					if ( obj != null && !obj.Completed && obj.Ingredient == Ingredient.StarChart )
					{
						...
						if ( hours < 5 || hours > 17 )
						{
							player.SendAsciiMessage( "You gaze up into the glittering night sky.  With great care, you compose a chart of the most prominent star patterns." );

							obj.Complete();
						}
						else
						{
							player.SendAsciiMessage( "You gaze up into the sky, but it is not dark enough to see any stars." );
						}
					}
				}
			}
