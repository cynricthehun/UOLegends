...
namespace Server.Items
{
	public class DisguiseKit : Item
	{
		...
		public bool ValidateUse( Mobile from )
		{
			...
			{
				// 
				from.SendAsciiMessage( "Only Members of the thieves guild are trained to use this item." );
			}
			else if ( Stealing.SuspendOnMurder && pm.Kills > 0 )
			{
				// 
				from.SendAsciiMessage( "You are currently suspended from the thieves guild.  They would frown upon your actions." );
			}
			...
			{
				from.SendAsciiMessage( "You cannot disguise yourself while holding a sigil" ); // 
			}
			else if ( TransformationSpell.UnderTransformation( from ) )
			{
				// 
				from.SendAsciiMessage( "You cannot disguise yourself while in that form." );
			}
			else if ( from.BodyMod == 183 || from.BodyMod == 184 )
			{
				// 
				from.SendAsciiMessage( "You cannot disguise yourself while wearing body paint" );
			}
	...
	public class DisguiseGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			{
				if ( m_Used )
					m_From.SendAsciiMessage( "Disguises wear off after 2 hours." ); // 
				else
					m_From.SendAsciiMessage( "You're looking good." ); // 
