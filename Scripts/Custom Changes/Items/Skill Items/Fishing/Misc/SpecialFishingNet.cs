...
namespace Server.Items
{
	public class SpecialFishingNet : Item
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( this.Name == null )
			{
				LabelTo( from, "a special fishing net" );
			}
			else
			{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( m_InUse )
			{
				from.SendAsciiMessage( "Someone is already using that net!" ); // 
			}
			else if ( IsChildOf( from.Backpack ) )
			{
				from.SendAsciiMessage( "Where do you wish to use the net?" ); // 
		...
		public void OnTarget( Mobile from, object obj )
		{
			...
			{
				from.SendAsciiMessage( "You need to be closer to the water to fish!" ); // 
			}
			else if ( FullValidation( map, x, y ) )
			{
				...
				from.SendAsciiMessage( "You plunge the net into the sea..." ); // 
			}
			else
			{
				from.SendAsciiMessage( "You can only use this net in deep water!" ); // 
			}
		}
