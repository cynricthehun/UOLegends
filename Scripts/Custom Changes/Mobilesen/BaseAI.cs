	...
	public abstract class BaseAI
	{
					...
					switch ( m_Order )
					{
						...
						case OrderType.Unfriend:
						{
							if ( m_Order == OrderType.Transfert && m_From.HasTrade )
								m_From.SendAsciiMessage( "You cannot transfer a pet with a trade pending." );
							else if ( m_Order == OrderType.Friend && m_From.HasTrade )
								m_From.SendAsciiMessage( "You cannot friend a pet with a trade pending." );
							else
		...
		public virtual void BeginPickTarget( Mobile from, OrderType order )
		{
			...
			if ( from.Target == null )
			{
				if ( order == OrderType.Transfert )
					from.SendAsciiMessage( "Click on the person to transfer ownership to." );
				else if ( order == OrderType.Friend )
					from.SendAsciiMessage( "Click on the player whom you wish to make a co-owner." );
				else if ( order == OrderType.Unfriend )
					from.SendAsciiMessage( "Click on the player whom you wish to remove as a co-owner." );
			...
			if ( e.Mobile.Alive && e.Mobile.InRange( m_Mobile.Location, 3 ) && m_Mobile.IsHumanInTown() )
			{
				...
				else if ( e.HasKeyword( 0x9E ) && WasNamed( e.Speech ) ) // *time*
				{
					...
					else
					{
						...
						Clock.GetTime( m_Mobile, out generalstring, out exactTime );

						//m_Mobile.PublicOverheadMessage( MessageType.Regular, 0x3B2, generalstring );
					}
				}
					...
					for ( int i = 0; i < keywords.Length; ++i )
					{
						...
						switch ( keyword )
						{
							...
							case 0x15B: // *friend
							{
								...
								{
									if ( m_Mobile.Summoned )
										e.Mobile.SendAsciiMessage( "Summoned creatures are loyal only to their summoners." );
									else if ( e.Mobile.HasTrade )
										e.Mobile.SendAsciiMessage( "You cannot friend a pet with a trade pending." );
									else
							...
							case 0x16E: // *transfer
							{
								...
								{
									if ( m_Mobile.Summoned )
										e.Mobile.SendAsciiMessage( "You cannot transfer ownership of a summoned creature." );
									else if ( e.Mobile.HasTrade )
										e.Mobile.SendAsciiMessage( "You cannot transfer a pet with a trade pending." );
									else
		...
		public virtual bool DoOrderFriend()
		{
			...
			else
			{
				...
				{
					from.SendAsciiMessage( "As a young player, you may not friend pets to older players." );
				}
				else if ( !youngFrom && youngTo )
				{
					from.SendAsciiMessage( "As an older player, you may not friend pets to young players." );
				}
				else if ( from.CanBeBeneficial( to, true ) )
				{
					...
					if ( fromState != null && toState != null )
					{
						if ( from.HasTrade )
						{
							from.SendAsciiMessage( "You cannot friend a pet with a trade pending." );
						}
						else if ( to.HasTrade )
						{
							to.SendAsciiMessage( "You cannot friend a pet with a trade pending." );
						}
						else if ( m_Mobile.IsPetFriend( to ) )
						{
							from.SendAsciiMessage( "That person is already a friend." );
						}
						else if ( !m_Mobile.AllowNewPetFriend )
						{
							from.SendAsciiMessage( "Your pet does not seem to be interested in making new friends right now." );
						}
						else
						{
							from.SendAsciiMessage( m_Mobile.Name + " will now accept movement commands from " + to.Name );

							to.SendAsciiMessage( from.Name + " has granted you the ability to give orders to their pet " + m_Mobile.Name + " This creature will now consider you as a friend." );
		...
		public virtual bool DoOrderUnfriend()
		{
			...
			{
				from.SendAsciiMessage( "That person is not a friend." );
			}
			else
			{
				from.SendAsciiMessage( m_Mobile.Name + " will no longer accept movement commands from " + to.Name );

				to.SendAsciiMessage( from.Name + " has no longer granted you the ability to give orders to their pet " + m_Mobile.Name + " This creature will no longer consider you as a friend." );
		...
		private class TransferItem : Item
		{
			...
			public override bool AllowSecureTrade( Mobile from, Mobile to, Mobile newOwner, bool accepted )
			{
				...
				{
					from.SendAsciiMessage( "As a young player, you may not transfer pets to older players." );
				}
				else if ( accepted && !youngFrom && youngTo )
				{
					from.SendAsciiMessage( "As an older player, you may not transfer pets to young players." );
				}
				else if ( accepted && !m_Creature.CanBeControlledBy( to ) )
				{
					from.SendAsciiMessage( "The pet refuses to be transferred because it will not obey " + to.Name );
					to.SendAsciiMessage( "The pet will not accept you as a master because it does not trust you." );

					return false;
				}
				else if ( accepted && !m_Creature.CanBeControlledBy( from ) )
				{
					from.SendAsciiMessage( "The pet refuses to be transferred because it will not obey you sufficiently." );
					to.SendAsciiMessage( "The pet will not accept you as a master because it does not trust " + from.Name + "." );
				}
				else if ( accepted && (to.Followers + m_Creature.ControlSlots) > to.FollowersMax )
				{
					to.SendAsciiMessage( "You have too many followers to control that creature." );
			...
			public override void OnSecureTrade( Mobile from, Mobile to, Mobile newOwner, bool accepted )
			{
				...
				if ( accepted )
				{
					if ( m_Creature.SetControlMaster( to ) )
					{
						...
						from.SendAsciiMessage( "You have transferred your pet to " + to.Name + "." );
						to.SendAsciiMessage( from.Name + " has transferred the allegiance of " + m_Creature.Name + " to you." );
					}
				}
			}
		}
			...
			if ( from != to && from != null && !from.Deleted && to != null && !to.Deleted && to.Player )
			{
				...
				else
				{
					...
					if ( fromState != null && toState != null )
					{
						if ( from.HasTrade )
						{
							from.SendAsciiMessage( "You cannot transfer a pet with a trade pending." );
						}
						else if ( to.HasTrade )
						{
							to.SendAsciiMessage( "You cannot transfer a pet with a trade pending." );
						}
						else
						{
		...
		public class InternalTimer : Timer
        {
            ...
            protected override void OnTick()
            {
                ...
                // an deactivated sector to an activated sector while the timer is running
		...
		private class AITimer : Timer
		{
			...
			protected override void OnTick()
			{
                ...
                (m_Owner.m_DeactivationTimer == null || !m_Owner.m_DeactivationTimer.Running)) //have to check this in the timer....
			    {
