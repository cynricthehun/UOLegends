...
namespace Arya.Chess
{
	...
	public class ChessControl : Item
	{
		...
		private int m_BlackMinorHue = 1109;
		private int m_WhiteMinorHue = 1001;
		...
		private static void BuildBoard( ChessControl chess, Point3D p, Map map )
		{
			...
			Server.Items.Static snw = new Server.Items.Static( stairNW );
			snw.Hue = 1175;
			snw.MoveToWorld( nw, map );
			Server.Items.Static sne = new Server.Items.Static( stairNE );
			sne.Hue = 1175;
			sne.MoveToWorld( ne, map );
			Server.Items.Static sse = new Server.Items.Static( stairSE );
			sse.Hue = 1175;
			sse.MoveToWorld( se, map );
			Server.Items.Static ssw = new Server.Items.Static( stairSW );
			ssw.Hue = 1175;
			ssw.MoveToWorld( sw, map );
			...
			for ( int x = 0; x < 8 * chess.m_SquareWidth; x++ )
			{
				...
				Server.Items.Static sn = new Server.Items.Static( stairN );
				sn.Hue = 1175;
				sn.MoveToWorld( top, map );
				Server.Items.Static ss = new Server.Items.Static( stairS );
				ss.Hue = 1175;
				ss.MoveToWorld( bottom, map );
				Server.Items.Static stairw = new Server.Items.Static( stairW );
				stairw.Hue = 1175;
				stairw.MoveToWorld( left, map );
				Server.Items.Static staire = new Server.Items.Static( stairE );
				staire.Hue = 1175;
				staire.MoveToWorld( right, map );
			}
		}
