	...
	public class VendorBackpack : Backpack
	{
		...
		public override bool CheckHold( Mobile m, Item item, bool message, bool checkItems, int plusItems, int plusWeight )
		{
			...
			if ( !BaseHouse.NewVendorSystem && Parent is PlayerVendor )
			{
				...
				{
					if ( message )
						m.SendAsciiMessage( "This action would exceed the secure storage limit of the house." );
		...
		public override void OnSingleClickContained( Mobile from, Item item )
		{
			if ( RootParent is PlayerVendor )
			{
				...
				{
					if ( !vi.IsForSale )
						item.LabelTo( from, "price: not for sale." ); // Price: Not for sale.
					else if ( vi.IsForFree )
						item.LabelTo( from, "price: free" ); // Price: FREE!
					else
						item.LabelTo( from, "price: {0}", vi.Price.ToString() ); // Price: ~1_COST~
	...
	public class PlayerVendor : Mobile
	{
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
			...
			if ( item is Gold )
			{
				if ( BaseHouse.NewVendorSystem )
				{
					...
					{
						from.SendAsciiMessage( "Your vendor has sufficient funds for operation and cannot accept this gold." );
		...
		private void OnItemGiven( Mobile from, Item item )
		{
			...
			if ( vi != null )
			{
				...
				from.SendAsciiMessage( "Type in a price and description for " + name + " (ESC=not for sale)" );
		...
		public bool CanInteractWith( Mobile from, bool ownerOnly )
		{
			...
			{
				from.SendAsciiMessage( "You can't shop from this home as you have been banned from this establishment." );
				return false;
			}
		...
		public int GiveGold( Mobile to, int amount )
		{
			...
			if ( amountGiven > 0 )
			{
				to.SendAsciiMessage( amountGiven.ToString() + " gold has been deposited into your bank box." );
			}
		...
		{
			from.SendAsciiMessage( "Enter a new name for your vendor (20 characters max):" );
		...
		{
			from.SendAsciiMessage( "Enter a new name for your shop (20 chars max):" );
		...
		public bool CheckTeleport( Mobile to )
		{
			...
			if ( Placeholder == null )
			{
				...
				to.SendAsciiMessage( "This vendor has been moved out of the house to your current location temporarily.  The vendor will return home automatically after two minutes have passed once you are done managing its inventory or customizing it." );
			}
			else
			{
				Placeholder.RestartTimer();

				to.SendAsciiMessage( "This vendor is currently temporarily in a location outside its house.  The vendor will return home automatically after two minutes have passed once you are done managing its inventory or customizing it." );
			}
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( e.HasKeyword( 0x3C ) || (e.HasKeyword( 0x171 ) && WasNamed( e.Speech ))  ) // vendor buy, *buy*
			{
				...
				{
					from.SendAsciiMessage( "Select the item you wish to buy." );
		...
		private class VendorNamePrompt : Prompt
		{
			...
			public override void OnResponse( Mobile from, string text )
			{
				...
				from.SendAsciiMessage( "Your vendor has been renamed." );
