	...
	public class HouseRegion : Region
	{
		...
		public override bool SendInaccessibleMessage( Item item, Mobile from )
		{
			if ( item is Container )
				item.SendAsciiMessageTo( from, "That is secure." );
			else
				item.SendAsciiMessageTo( from, "You are not allowed to access this." );
		...
		public override void OnLocationChanged( Mobile m, Point3D oldLocation )
		{
			...
			else if ( (m_House.Public || !m_House.IsAosRules) && m_House.IsBanned( m ) && m_House.IsInside( m ) )
			{
				...
				m.SendAsciiMessage( "You may not enter." );
			}
			...
			{
				m.Location = m_House.BanLocation;
				m.SendAsciiMessage( "You are not allowed to access this." );
			}
		...
		public override bool OnMoveInto( Mobile from, Direction d, Point3D newLocation, Point3D oldLocation )
		{
			...
			else if ( (m_House.Public || !m_House.IsAosRules) && m_House.IsBanned( from ) && m_House.IsInside( newLocation, 16 ) )
			{
				...
				from.SendAsciiMessage( "You may not enter." );
				return false;
			}
			...
			{
				from.SendAsciiMessage( "You are not allowed to access this." );
				return false;
			}
			else if ( m_House is HouseFoundation )
			{
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( e.HasKeyword( 0x33 ) ) // remove thyself
			{
				if ( isFriend )
				{
					from.SendAsciiMessage( "Target the individual to eject from this house." );
					from.Target = new HouseKickTarget( m_House );
				}
				else
				{
					from.SendAsciiMessage( "You must be in your house to do this." );
				}
			}
			else if ( e.HasKeyword( 0x34 ) ) // I ban thee
			{
				...
				{
					from.SendAsciiMessage( "You cannot ban someone from a private house. Revoke their access instead." );
				}
				else
				{
					from.SendAsciiMessage( "Target the individual to ban from this house." );
			...
			else if ( e.HasKeyword( 0x23 ) ) // I wish to lock this down
			{
				if ( isCoOwner )
				{
					from.SendAsciiMessage( "Lock what down?" );
					from.Target = new LockdownTarget( false, m_House );
				}
				else if ( isFriend )
				{
					from.SendAsciiMessage( "You are not a co-owner of this house." );
				}
				else
				{
					from.SendAsciiMessage( "You must be in your house to do this." );
			...
			else if ( e.HasKeyword( 0x24 ) ) // I wish to release this
			{
				if ( isCoOwner )
				{
					from.SendAsciiMessage( "Choose the item you wish to release." );
			...
			else if ( e.HasKeyword( 0x25 ) ) // I wish to secure this
			{
				if ( isOwner )
				{
					from.SendAsciiMessage( "Choose the item you wish to secure." );
			...
			else if ( e.HasKeyword( 0x26 ) ) // I wish to unsecure this
			{
				if ( isOwner )
				{
					from.SendAsciiMessage( "Choose the item you wish to unsecure." );
			...
			else if ( e.HasKeyword( 0x27 ) ) // I wish to place a strong box
			{
				if ( isOwner )
				{
					from.SendAsciiMessage( "Owners do not get a strongbox of their own." );
				}
				else if ( isCoOwner )
				{
		...
		public override bool OnSingleClick( Mobile from, object o )
		{
			if ( o is Item )
			{
				...
				if ( m_House.IsLockedDown( item ) )
					item.LabelTo( from, "[locked down]" ); // [locked down]
				else if ( m_House.IsSecure( item ) )
					item.LabelTo( from, "[locked down & secure]" ); // [locked down & secure]
			}
