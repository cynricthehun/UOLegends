		...
		private class MistletoeAddonGump : Gump
		{
			...
			public override void OnResponse( NetState sender, RelayInfo info )
			{
				if ( info.ButtonID == 1 )
				{
					...
					{
						m_From.SendAsciiMessage( "You are too far away to do that." );
					}
				}
			}
		}
	...
	public class MistletoeDeed : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsChildOf( from.Backpack ) )
			{
				...
				if ( house != null && house.IsCoOwner( from ) )
				{
					from.SendAsciiMessage( "Where would you like to place this decoration?" );
		...
		private void PlaceAddon( Mobile from, Point3D loc, bool northWall, bool westWall )
		{
			...
			if ( northWall )
				...
				from.SendAsciiMessage( "The mistletoe must be placed next to a wall." );
