	...
	public class RenounceYoungGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 1 )
			{
				...
				if ( acc != null )
				{
					acc.RemoveYoungStatus( "You have chosen to renounce your `Young' player status." );
				}
			}
			else
			{
				from.SendAsciiMessage( "You have chosen not to renounce your `Young' player status." );
			}
		}
	}
}
