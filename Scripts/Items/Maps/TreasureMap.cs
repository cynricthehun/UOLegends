...
namespace Server.Items
{
	public class TreasureMap : MapItem
	{
		...
		public override void OnSingleClick( Mobile from )
		{
			if ( m_Completed )
			{
				from.Send( new AsciiMessage( this.Serial, 0, MessageType.Label, 0x3B2, 3, null, m_CompletedBy != null ? "a treasure map completed by " + m_CompletedBy.Name : "a treasure map completed by someone") );
			}
			else if ( m_Decoder != null )
			{
				if ( m_Level == 0 )
					LabelTo( from, "a youthful treasure map" );
				if ( m_Level == 1 )
					LabelTo( from, "a plainly drawn treasure map" );
				if ( m_Level == 2 )
					LabelTo( from, "an expertly drawn treasure map" );
				if ( m_Level == 3 )
					LabelTo( from, "a adeptly drawn treasure map" );
				if ( m_Level == 4 )
					LabelTo( from, "a cleverly drawn treasure map" );
				if ( m_Level == 5 )
					LabelTo( from, "a deviously drawn treasure map" );
				if ( m_Level == 6 )
					LabelTo( from, "an ingeniously drawn treasure map" );
			}
			else
			{
				if ( m_Level == 0 )
					LabelTo( from, "a tattered, youthful treasure map" );
				if ( m_Level == 1 )
					LabelTo( from, "a tattered, plainly drawn treasure map" );
				if ( m_Level == 2 )
					LabelTo( from, "a tattered, expertly drawn treasure map" );
				if ( m_Level == 3 )
					LabelTo( from, "a tattered, adeptly drawn treasure map" );
				if ( m_Level == 4 )
					LabelTo( from, "a tattered, cleverly drawn treasure map" );
				if ( m_Level == 5 )
					LabelTo( from, "a tattered, deviously drawn treasure map" );
				if ( m_Level == 6 )
					LabelTo( from, "a tattered, ingeniously drawn treasure map" );
