...
namespace Server.Mobiles
{
	...
	public class XmlAddGump : Gump
	{
		...
		public XmlSpawner m_LastSpawner;
		...
		public static void XmlAdd_OnCommand( CommandEventArgs e )
		{
			...
			e.Mobile.SendGump( new XmlAddGump(e.Mobile, e.Mobile.Location, e.Mobile.Map, true, false, null,x,y));

		}

		public XmlAddGump(Mobile from, Point3D startloc, Map startmap) : this(from, startloc, startmap, null)
		{
		}

		public XmlAddGump(Mobile from, Point3D startloc, Map startmap, string entry) : this(from, startloc, startmap, true, false, null, StartingXoffset, 0)
		{
		}

		public XmlAddGump( Mobile from, Point3D startloc, Map startmap, bool firststart, bool extension, XmlSpawner lastspawner, int gumpx, int gumpy) : base( gumpx,gumpy )
		{
			if(from == null) return;
		...
		private void DoShowGump(XmlSpawner x)
		{
			if(m_From == null || m_From.Deleted) return;
		...
		public void Refresh(Mobile from)
		{
			...
			from.SendGump( new XmlAddGump(this.m_From, this.StartingLoc, this.StartingMap, false, this.m_ShowExtension, this.m_LastSpawner, x,y));
		}
		

		private class PlaceSpawnerTarget : Target
		{
			XmlAddGump m_addgump;
			XmlSpawnerDefaults.DefaultEntry defs;
			NetState m_state;

			public PlaceSpawnerTarget(XmlAddGump addgump, NetState state) :  base ( 30, true, TargetFlags.None )
			{
				m_addgump = addgump;
				defs = m_addgump.defs;
				m_state = state;
			...
			{
				if(defs == null || from == null || m_addgump == null) return;
				...
				m_addgump.m_LastSpawner = spawner;

				// open the spawner gump and the xmladdgump
				m_addgump.DoShowGump(m_addgump.m_LastSpawner);
				...
				from.CloseGump(typeof(XmlAddGump));
				m_addgump.Refresh(m_state.Mobile);
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
				...
				{
					state.Mobile.Target = new PlaceSpawnerTarget(this, state);
				...
				{
					if(m_LastSpawner == null || m_LastSpawner.Deleted) break;
					Refresh(state.Mobile);
					state.Mobile.SendGump( new XmlAddConfirmDeleteGump(state.Mobile, m_LastSpawner) );
					return;
				}
				case 157: // Reset last spawner
				{
					if(m_LastSpawner != null && !m_LastSpawner.Deleted)
						m_LastSpawner.DoReset = true;
					break;
				}
				case 158: // Respawn last spawner
				{
					if(m_LastSpawner != null && !m_LastSpawner.Deleted)
						m_LastSpawner.DoRespawn = true;
					break;
				}
				case 180: // Set Options
				{
					Refresh(state.Mobile);
					state.Mobile.SendGump( new XmlAddOptionsGump(state.Mobile, m_LastSpawner) );
					return;
				}
				...
				{
					// then go to it
					DoGoTo(m_LastSpawner);
					break;
				}
				case 1001:  // Show Gump
				{
					Refresh(state.Mobile);
					DoShowGump(m_LastSpawner);
					break;
				}
				case 1002:  // Show Props
				{
					Refresh(state.Mobile);
					DoShowProps(m_LastSpawner);
					break;
				}
				...
				default:
				{
					...
					if(info.ButtonID >= 5000 && info.ButtonID < 5000+ MaxEntries)
					{
						...
						*/
						...
						XmlAddGump newg = new XmlAddGump(state.Mobile, StartingLoc, StartingMap, false, m_ShowExtension, m_LastSpawner, 0,0);
