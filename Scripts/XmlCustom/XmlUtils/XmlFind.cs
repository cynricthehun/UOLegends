	...
	public class XmlFindGump : Gump
	{
		...
		public class XmlFindThread
		{
			...
			public void XmlFindThreadMain()
			{
				...
				Timer.DelayCall(TimeSpan.Zero, new TimerStateCallback(GumpDisplayCallback), new object[] { m_From, gump });
		...
		{
			Server.Commands.Register("XmlFind", AccessLevel.GameMaster, new CommandEventHandler(XmlFind_OnCommand));
		}
		...
		private static bool TestValidInternal(object o)
		{
			if (o is Mobile)
			{
				...
				if (o is Item)
				{
					...
					if (i.Map != Map.Internal || i.Parent != null || i is Fists || i is MountItem || i is EffectItem || i.HeldBy != null /*|| i is ICommodity */||
		...
		public static ArrayList Search(SearchCriteria criteria, out string status_str)
		{
			...
			if (criteria == null) return newarray;
			...
			if (!criteria.Dosearchattach)
			{
				...
				catch { status_str = "Unable to search World.Items"; }
				...
				if (itemarray != null)
				{
					foreach (Item i in itemarray)
					{
						...
						// need to keep track of valid internalized commodity deed items
						// this will deal with valid commodity deeds that are not on the internal map
						if (i is CommodityDeed && criteria.Dohidevalidint && i.Map != Map.Internal && i.Map != null)
						{
						...
						// need to keep track of valid internalized commodity deed items
						// this will deal with valid commodity deeds that are on the internal map
						if (i is CommodityDeed && criteria.Dohidevalidint && hasvalidhidden)
						{
							CommodityDeed deed = (CommodityDeed)i;
						...
						// also keep track of valid internalized relocatable house items
						if (i is BaseHouse && criteria.Dohidevalidint && hasvalidhidden)
						{
						...
						{
							Type targetentrytype = SpawnerType.GetType(criteria.Searchspawnentry.ToLower());

							if (criteria.Searchspawnentry == null || targetentrytype == null)
							{
								hasentry = false;
							}
							else
			...
			if (!criteria.Dosearcherr && !criteria.Dosearchattach)
			{
				...
				catch { status_str = "Unable to search World.Mobiles"; }
