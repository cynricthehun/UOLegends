...
namespace Server.Mobiles
{
	public class HelpGump : Gump
	{
		...
		public HelpGump( XmlSpawner spawner, XmlSpawnerGump spawnergump, int X, int Y ) : base( X, Y )
		{
			...
			AddHtml( 25, 20, width-10,440,
				...
				"SETONCARRIED,itemname[,itemtype]/property/value/...\n" +
	...
	public class TextEntryGump : Gump
	{
		...
		public TextEntryGump( XmlSpawner spawner, XmlSpawnerGump spawnergump, int index, int X, int Y ) : base( X, Y )
		{
			if(spawner == null || spawner.Deleted)
				return;
			m_Spawner = spawner;
	...
	public class XmlSpawnerGump : Gump
	{
		...
		public XmlSpawnerGump( XmlSpawner spawner , int X, int Y, int extension, int textextension, int newpage) : this( spawner, X, Y, extension, textextension, newpage, null )
		{
		}

		public XmlSpawnerGump( XmlSpawner spawner , int X, int Y, int extension, int textextension, int newpage, ReplacementEntry rentry) : base( X, Y )
		{
			if(spawner == null || spawner.Deleted)
				return;
		...
		public static void ProcessSpawnerBookEntry(Mobile from, object[] args, string entry)
		{
			...
			if(m_Index < m_Spawner.SpawnObjects.Length)
			{
				...
				if(m_Spawner.SpawnObjects[m_Index].TypeName != entry)
				{
					CommandLogging.WriteLine(from, "{0} {1} changed XmlSpawner {2} '{3}' [{4}, {5}] ({6}) : {7} to {8}", from.AccessLevel, CommandLogging.Format( from ),  m_Spawner.Serial, m_Spawner.Name, m_Spawner.GetWorldLocation().X, m_Spawner.GetWorldLocation().Y, m_Spawner.Map, m_Spawner.SpawnObjects[m_Index].TypeName, entry);
