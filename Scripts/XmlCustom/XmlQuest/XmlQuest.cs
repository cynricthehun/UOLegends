	...
	public abstract class XmlQuest
	{
			...
			if (arglist.Length > argstart)
			{
				...
				for (int i = argstart; i < arglist.Length; i++)
				{
					...
					if (propargs.Length > 1)
					{
						...
						if (arglist[i] != null && arglist[i].Length > 0 && arglist[i][0] >= '0' && arglist[i][0] <= '9')
						{
							// its a count arg
							try
							{
		...
		public static void ApplyCollected(Item target, IXmlQuest quest)
		{
			...
			if (!quest.Completed1 && CheckCollectObjective(quest, target, quest.Objective1, quest.State1, out newstatestr, out collectstatus))
			{
				...
				if (!quest.Completed2 && CheckCollectObjective(quest, target, quest.Objective2, quest.State2, out newstatestr, out collectstatus))
				{
					quest.State2 = newstatestr;
					quest.Completed2 = collectstatus;
				}
				else
					if (!quest.Completed3 && CheckCollectObjective(quest, target, quest.Objective3, quest.State3, out newstatestr, out collectstatus))
					{
						quest.State3 = newstatestr;
						quest.Completed3 = collectstatus;
					}
					else
						if (!quest.Completed4 && CheckCollectObjective(quest, target, quest.Objective4, quest.State4, out newstatestr, out collectstatus))
						{
							quest.State4 = newstatestr;
							quest.Completed4 = collectstatus;
						}
						else
							if (!quest.Completed5 && CheckCollectObjective(quest, target, quest.Objective5, quest.State5, out newstatestr, out collectstatus))
							{
		...
		public static bool ApplyGiven(Mobile mob, Item target, IXmlQuest quest)
		{
			...
			if (!quest.Completed1 && CheckGiveObjective(quest, mob, target, quest.Objective1, quest.State1, out newstatestr, out givestatus))
			{
				...
				if (!quest.Completed2 && CheckGiveObjective(quest, mob, target, quest.Objective2, quest.State2, out newstatestr, out givestatus))
				{
					quest.State2 = newstatestr;
					...
					if (!quest.Completed3 && CheckGiveObjective(quest, mob, target, quest.Objective3, quest.State3, out newstatestr, out givestatus))
					{
						quest.State3 = newstatestr;
						...
						if (!quest.Completed4 && CheckGiveObjective(quest, mob, target, quest.Objective4, quest.State4, out newstatestr, out givestatus))
						{
							quest.State4 = newstatestr;
							...
							if (!quest.Completed5 && CheckGiveObjective(quest, mob, target, quest.Objective5, quest.State5, out newstatestr, out givestatus))
							{
								quest.State5 = newstatestr;
		...
		public static void ApplyKilled(Mobile m_killed, Mobile m_killer, IXmlQuest quest)
		{
			...
			if (!quest.Completed1 && CheckKillObjective(quest, m_killed, m_killer, quest.Objective1, quest.State1, out newstatestr, out killstatus))
			{
				...
				if (!quest.Completed2 && CheckKillObjective(quest, m_killed, m_killer, quest.Objective2, quest.State2, out newstatestr, out killstatus))
				{
					quest.State2 = newstatestr;
					quest.Completed2 = killstatus;
				}
				else
					if (!quest.Completed3 && CheckKillObjective(quest, m_killed, m_killer, quest.Objective3, quest.State3, out newstatestr, out killstatus))
					{
						quest.State3 = newstatestr;
						quest.Completed3 = killstatus;
					}
					else
						if (!quest.Completed4 && CheckKillObjective(quest, m_killed, m_killer, quest.Objective4, quest.State4, out newstatestr, out killstatus))
						{
							quest.State4 = newstatestr;
							quest.Completed4 = killstatus;
						}
						else
							if (!quest.Completed5 && CheckKillObjective(quest, m_killed, m_killer, quest.Objective5, quest.State5, out newstatestr, out killstatus))
							{
								quest.State5 = newstatestr;
			...
			for (int i = 0; i < mobitems.Count; i++)
			{
				...
				if (mobitems[i] is IXmlQuest)
				{
					...
					if (quest != null && !quest.Deleted && quest.PartyEnabled)
					{
						...
						if (member != null && !member.Deleted && member == m_killer && quest != null && !quest.Deleted)
						{
		...
		public static void ApplyEscorted(Mobile m_escorted, Mobile m_escorter, IXmlQuest quest)
		{
			...
			if (!quest.Completed1 && CheckEscortObjective(quest, m_escorted, m_escorter, quest.Objective1, quest.State1, out newstatestr, out escortstatus))
			{
				...
				if (!quest.Completed2 && CheckEscortObjective(quest, m_escorted, m_escorter, quest.Objective2, quest.State2, out newstatestr, out escortstatus))
				{
					quest.State2 = newstatestr;
					quest.Completed2 = escortstatus;
				}
				else
					if (!quest.Completed3 && CheckEscortObjective(quest, m_escorted, m_escorter, quest.Objective3, quest.State3, out newstatestr, out escortstatus))
					{
						quest.State3 = newstatestr;
						quest.Completed3 = escortstatus;
					}
					else
						if (!quest.Completed4 && CheckEscortObjective(quest, m_escorted, m_escorter, quest.Objective4, quest.State4, out newstatestr, out escortstatus))
						{
							quest.State4 = newstatestr;
							quest.Completed4 = escortstatus;
						}
						else
							if (!quest.Completed5 && CheckEscortObjective(quest, m_escorted, m_escorter, quest.Objective5, quest.State5, out newstatestr, out escortstatus))
							{
								quest.State5 = newstatestr;
			...
			for (int i = 0; i < mobitems.Count; i++)
			{
				...
				if (mobitems[i] is IXmlQuest)
				{
					...
					if (quest != null && !quest.Deleted && quest.PartyEnabled)
					{
						...
						if (member != null && !member.Deleted && member == m_escorter && quest != null && !quest.Deleted)
						{
		...
		public static void ApplyVisited(PlayerMobile m_player, IXmlQuest quest)
		{
			...
			if (!quest.Completed1 && CheckVisitObjective(quest, m_player, quest.Objective1, quest.State1, out newstatestr, out visitstatus))
			{
				...
				if (!quest.Completed2 && CheckVisitObjective(quest, m_player, quest.Objective2, quest.State2, out newstatestr, out visitstatus))
				{
					quest.State2 = newstatestr;
					quest.Completed2 = visitstatus;
				}
				else
					if (!quest.Completed3 && CheckVisitObjective(quest, m_player, quest.Objective2, quest.State2, out newstatestr, out visitstatus))
					{
						quest.State3 = newstatestr;
						quest.Completed3 = visitstatus;
					}
					else
						if (!quest.Completed4 && CheckVisitObjective(quest, m_player, quest.Objective4, quest.State4, out newstatestr, out visitstatus))
						{
							quest.State4 = newstatestr;
							quest.Completed4 = visitstatus;
						}
						else
							if (!quest.Completed5 && CheckVisitObjective(quest, m_player, quest.Objective5, quest.State5, out newstatestr, out visitstatus))
							{
								quest.State5 = newstatestr;
