...
namespace Server.Mobiles
{
	...
	public class XmlSpawner : Item
	{
		...
		public enum SpawnPositionType { Random, RowFill, ColFill, Perimeter, Player, Waypoint, RelXY, DeltaLocation, Location }
		...
		private const int defaultTriggerSound = 0x1F4;          // click and sparkle sound by default  (0x1F4) , click sound (0x3A4)
		public const string XmlSpawnDir = "Spawns";            // default directory for saving/loading .xml files with [xmlload [xmlsave
		private const string XmlConfigsDir = "SpawnerConfigs";  // default directory for loading .xml config files with LoadConfig
		...
		private const int MaxMoveCheck = 10; // limit number of players that can be checked for triggering in a single OnMovement tick
		...
		private bool m_regionUpdated = false;
		...
		*/
				...
				foreach (SpawnObject so in m_SpawnObjects)
				{
					for (int x = 0; x < so.SpawnedObjects.Count; x++)
					{
						...
						if (o is BaseCreature)
						{
							...
							if ((b.Hits < b.HitsMax) || (b.Map != this.Map))
							{
								return true;
							}
		...
		public bool HasActiveSectors
		{
			get
			{
				...
				foreach (Sector s in sectorList)
				{
					...
					if (s != null && s.Active && s.Players.Count > 0)
					{
		...
		{
			set { if (value == true) m_SpawnerGump = null; }
		}
		...
		public void ReportStatus()
		{
			...
			if (tokunomap != null)
			{
				...
				for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; i++)
				{
					...
					if (XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, tokunomap).Count > 0)
						Console.WriteLine("\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, tokunomap).Count);

					count += XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, tokunomap).Count;
				}
			}
...
public static void _TraceEnd(int index)
{
	   if(index < MaxTraces){
		...
		{
			private CommandEventArgs m_e;
			public InternalTarget(CommandEventArgs e)
				: base(30, false, TargetFlags.None)
			{
				m_e = e;
			}
		...
		{
			e.Mobile.Target = new InternalTarget(e);
		}
		...
		public static void OptimalSmartSpawning_OnCommand(CommandEventArgs e)
		{
			...
			foreach (Item item in World.Items.Values)
			{
				if (item is XmlSpawner)
				{
					...
					if (spawner.m_SpawnObjects != null) nso = spawner.SpawnObjects.Length;
					...
					for (int i = 0; i < nso; ++i)
					{
						string typestr = spawner.SpawnObjects[i].TypeName;
				...
				if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
				{
					foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
					{
						...
						if (SpawnerPrefix == null || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
						{
							...
							else if (string.Compare(XmlMapName, "Tokuno", true) == 0)
							{
								try
								{
		...
		private static void ImportSpawner(XmlElement node, Mobile from)
		{
			...
			// Create the new xml spawner
			XmlSpawner spawner = new XmlSpawner(SpawnId, location.X, location.Y, 0, 0, name, count,
		...
		private static void SaveSpawns(CommandEventArgs e, bool SaveAllMaps, bool oldformat)
		{
			...
			foreach (Item i in World.Items.Values)
			{
				if (((SaveAllMaps == true) || (i.Map == e.Mobile.Map)) &&
					...
					&& (SpawnerPrefix == null || (SpawnerPrefix.Length == 0) || (i.Name.StartsWith(SpawnerPrefix))))
				{
					saveslist.Add(i);
				}
			}
		...
		public static bool SaveSpawnList(Mobile from, ArrayList savelist, string dirname, Stream stream, bool oldformat, bool verbose)
		{
			...
			foreach (XmlSpawner sp in savelist)
			{
				...
				if (string.Compare(sp.Map.Name, Map.Trammel.Name, true) == 0)
					...
					MalasCount++;
				else if (string.Compare(sp.Map.Name, "Tokuno", true) == 0)
					TokunoCount++;
				else
					OtherCount++;
			...
			foreach (SpawnObject so in m_SpawnObjects)
			{
				for (int x = 0; x < so.SpawnedObjects.Count; x++)
				{
					...
					if (o is Item)
					{
						...
						// enable this check
						if (item.Deleted || (item.Parent != this.Parent) // different container
					...
					else if (o is Mobile)
					{
							...
							&& m.Map != null && m.Map != Map.Internal && !m.Map.GetSector(m.Location).Active)
						{
							...
							m.Location = GetSpawnPosition(so.RequireSurface);
		...
		private int RandomAvailableSpawnIndex(int sgroup)
		{
			...
			for (int i = 0; i < m_SpawnObjects.Count; i++)
			{
				...
				{
					// keep track of the number of the total spawn objects in the group
					ngroup++;
					// keep track of the number of spawn objects that are not at max (hence available for spawning)
					if (s.MaxCount > s.SpawnedObjects.Count)
					{
						avail++;
			...
			if (avail > 0 && maxrange > 0 && (avail == ngroup || sgroup <= 0))
			{
		...
		public void OnTick()
		{
			...
			// update the region name in case new regions were added after the initialization pass
			if (!m_regionUpdated && RegionName != null && RegionName != string.Empty)
			{
				RegionName = RegionName;

				m_regionUpdated = true;
			}
		...
		{
			return Spawn(index, smartspawn, count, -1, Point3D.Zero);
		}
		...
		{
			// no pack spawning
			return GetSpawnPosition(requiresurface, -1, Point3D.Zero, null);
		}
		...
		public void DeleteSpawnObject(Mobile from, string SpawnObjectName)
		{
			...
			try
			{
				...
				if (TheSpawn != null)
				{
					...
					while (TheSpawn.SpawnedObjects.Count > TheSpawn.MaxCount)
					{
		...
		private class InternalTimer2 : Timer
		{
			...
			public InternalTimer2(XmlSpawner spawner, TimeSpan delay)
				: base(delay)
			{

				// reduce timer priority if spawner is full, reduces the timer overhead for most spawners
				if (spawner.IsFull)
				{
					Priority = TimerPriority.FiveSeconds;
					// reset the spawner countdown
			...
			public SpawnObject(Mobile from, XmlSpawner spawner, string name, int maxamount)
			{
				...
				if (from != null && spawner != null)
				{
					...
					if (spawner.SpawnObjects != null)
					{
						...
						{
							SpawnObject s = spawner.SpawnObjects[i];
